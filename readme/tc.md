...
另外在你**右手手背**上會浮著一個小面板。看著按鈕一會兒就會按下去。上面有
**PIN**(不用切回桌面就能重新擺放提示條)和 **STYLE**。
...
**如果你的 VRChat 沒有安裝在預設位置**,程式會找不到日誌,只能退而用聲音辨識曲目 ——
而聲音無法分辨那些用同一首音樂、但編舞不同的曲目。請編輯程式旁邊的 `config.toml`,
找到 `[log]` 這一段:

```toml
[log]
dir = '%USERPROFILE%\AppData\LocalLow\VRChat\VRChat'   # <- 改這一行
```

把它指向存放 `output_log_*.txt` 的資料夾。**單引號要保留** —— 它可以避免反斜線被當成跳脫字元。
存檔後重新啟動程式即可。
