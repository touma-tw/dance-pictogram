...
为 VRChat 的 [PyPyDance](https://vrchat.com/home/world/wrld_f20326da-f1ac-45fc-a062-609723b097b1/info) 而做。
...
另外在你**右手手背**上会浮着一个小面板。看着按钮一会儿就会按下去。上面有
**PIN**(不用切回桌面就能重新摆放提示条)和 **STYLE**。
...
**如果你的 VRChat 没有安装在默认位置**,程序会找不到日志,只能退而用声音识别曲目 ——
而声音无法分辨那些用同一首音乐、但编舞不同的曲目。请编辑程序旁边的 `config.toml`,
找到 `[log]` 这一段:

```toml
[log]
dir = '%USERPROFILE%\AppData\LocalLow\VRChat\VRChat'   # <- 改这一行
```

把它指向存放 `output_log_*.txt` 的文件夹。**单引号要保留** —— 它可以避免反斜杠被当成转义字符。
保存后重新启动程序即可。
